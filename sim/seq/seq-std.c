...
int gen_instr_valid()
{
    return ((icode) == (I_NOP)||(icode) == (I_HALT)||(icode) == (I_RRMOVL)||(icode) == (I_IRMOVL)||(icode) == (I_RMMOVL)||(icode) == (I_MRMOVL)||(icode) == (I_ALU)||(icode) == (I_ALUI)||(icode) == (I_JXX)||(icode) == (I_CALL)||(icode) == (I_RET)||(icode) == (I_PUSHL)||(icode) == (I_POPL));
}
...
int gen_srcB()
{
    return (((icode) == (I_ALU)||(icode) == (I_ALUI)||(icode) == (I_RMMOVL)||(icode) == (I_MRMOVL)) ? (rb) : ((icode) == (I_PUSHL)||(icode) == (I_POPL)||(icode) == (I_CALL)||(icode) == (I_RET)) ? (REG_ESP) : 1 ? (REG_NONE) : 0);
}

int gen_dstE()
{
    return (((icode) == (I_RRMOVL)||(icode) == (I_IRMOVL)||(icode) == (I_ALU)||(icode) == (I_ALUI)) ? (rb) : ((icode) == (I_PUSHL)||(icode) == (I_POPL)||(icode) == (I_CALL)||(icode) == (I_RET)) ? (REG_ESP) : 1 ? (REG_NONE) : 0);
}
...
int gen_aluA()
{
    return ((((icode) == (I_IRMOVL)||(icode) == (I_ALUI)) & ((ra) == (REG_NONE))) ? (valc) : ((icode) == (I_RRMOVL)||(icode) == (I_ALU)||(icode) == (I_ALUI)) ? (vala) : ((icode) == (I_RMMOVL)||(icode) == (I_MRMOVL)) ? (valc) : ((icode) == (I_CALL)||(icode) == (I_PUSHL)) ? -4 : ((icode) == (I_RET)||(icode) == (I_POPL)) ? 4 : 0);
}

int gen_aluB()
{
    return (((icode) == (I_RMMOVL)||(icode) == (I_MRMOVL)||(icode) == (I_ALU)||(icode) == (I_ALUI)||(icode) == (I_CALL)||(icode) == (I_PUSHL)||(icode) == (I_RET)||(icode) == (I_POPL)) ? (valb) : ((icode) == (I_RRMOVL)||(icode) == (I_IRMOVL)) ? 0 : 0);
}
